...
def job_buildpipe():
    json_ = request.json
    yay='https://github.com/lekhana3003/jen1.git'
    job_name=json_['repo_name']+'pipeline'
    fhand=open('examples/addpipline.xml','r')
    xml=fhand.read()
    jenkins = Jenkins(
    'http://localhost:8080', username='hardik', password='test123',
    requester=CrumbRequester(
    ...
    my_job = jenkins[job_name]
    myConfig=my_job.get_config()
    rep = myConfig.replace("<url>https://github.com/lekhana3003/abcd.git</url>", "<url>"+yay+"</url>")
    my_job.update_config(rep)
    jenkins.build_job(job_name)
